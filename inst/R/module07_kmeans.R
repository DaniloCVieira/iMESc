...
k_means_module$ui<-function(id){
  module_progress("Loading module: K-Means")
  ns<-NS(id)
  div(

    h4("K-Means", class="imesc_title"),
        ...
        div(style="display: flex;column-gap: 20px;flex-flow: wrap  ;flex-grow: 4;",class="setup_box setup_top half-drop-inline;",
            ...
            div(
              ...
              div(class="save_changes",
                  tipify_ui(actionButton(ns("save_kmeans_models"), icon("fas fa-save"),style="margin-right: 10px"),"Save K-Means Model in the training Datalist X")
              )
            )
  ...
  moduleServer(id,function(input, output, session){
    ...
    output$psom_out<-renderUI({
      ...
      renderPlot({
        ...
        bg_color<-colorspace::lighten(bg_color,args$bgalpha)
    ...
    get_elbow<-eventReactive(input$oc_run,{
      ...
      p<-factoextra::fviz_nbclust(getdata_kmeans(), kmeans, method="wss", k.max=input$k.max,iter.max=input$km_itermax, nstart=input$km_nstart,algorithm=km_alg()) + theme_minimal() + ggtitle("the Elbow Method")
      p
    ...
    observeEvent(ignoreInit=T,input$oc_run,{
      ...
      output$oc_elbow_out<-renderUI({
        req(input$k.max)
        renderPlot({
          p<-factoextra::fviz_nbclust(getdata_kmeans(), kmeans, method="silhouette", k.max=input$k.max,iter.max=input$km_itermax, nstart=input$km_nstart,algorithm=km_alg()) + theme_minimal() + ggtitle("the Silhouette Method")
          vals$kmeans_elbow<-p
          vals$kmeans_elbow
        })
      })
    ...
    savekmeans<-reactive({
      ...
      shinyjs::removeClass('save_kmeansbtn','save_changes')
      #shinyjs::hide('save_kmeans')
    })

    observeEvent(ignoreInit=T,input$kmeans_run,{
      shinyjs::addClass('save_kmeansbtn','save_changes')
    })
