    ...
    modalDialog(
      div(
        div(style="background: white; margin-left: 20px;padding-top: 10px",class="half-drop dl",
            div(id=ns("dl_page1"),
                h4("Create Datalist",
                   tiphelp_icon(icon("fas fa-question-circle"),"Each Datalist in the Databank requires at least one file for the Numeric-Attribute. If a Factor-Attribute is not provided, iMESc automatically generates this attribute as a single-column sheet containing the IDs of the Numeric-Attribute.")),

                div(class="insert_radio",
                    radioButtons(ns("up_or_ex"),NULL,choiceValues  = list("upload", "example"),choiceNames   = list(tiphelp_icon(strong("Upload"),"upload your own data",placement = "bottom"), tiphelp_icon(strong("example"),"Use Nematode datasets from Araca Bay as example",placement = "bottom")),selected = "upload", inline = T)),
...
tool1$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ...
    validate_data<-reactive({
      ...
      req(path)
      if(grepl(".csv",path)){
        data<-data.frame(data.table::fread(path, stringsAsFactors = T,na.strings=c("","NA"),header=T,select=1))
        ...
        df<-data.frame(readxl::read_excel(path, sheet,na=c("","NA"),col_names =F, range = readxl::cell_cols(1)))
...
tool2_tab3$ui<-function(id){
  ...
  div(style="height: calc(100vh - 100px);",
      ...
      div(id = ns('step1'),
          ...
          checkboxInput(ns("replace"), span("Replace", tipright("If true, add as new with unique names; else, replace matching columns in the destination."))),
  ...
  moduleServer(id,function(input,output,session){
    ...
    output$cut_fac_method<-renderUI({
      ...
      div(class="half-drop half-drop-inline",
          selectInput(
            ns("bin_method"),
            label = span("Bin method",tipify_ui(actionLink(ns("bin_method_help"), icon("fas fa-question-circle")), "Click for details")
            ),
    ...
    prep_exchange_fun<-function(from,to,afrom,ato,saved_data,r_exchange,replace=F){
      ...
      if(ato=="factor"){
        ...
        if(isTRUE(replace)){
          newfac<-factors
          newfac[colnames(r_exchange)]<-r_exchange
        } else{
          newfac<-cbind(factors,r_exchange)
        }
        ...
        if(isTRUE(replace)){
          new[colnames(r_exchange)]<-r_exchange
          newdat<-new
        } else{
    ...
    prep_exchange<-reactive({
      try({
        ...
        result<-capture_log2(prep_exchange_fun)(from=input$import_from_data,to=input$import_to_data,afrom=input$import_from_attr,ato=input$import_to_attr,saved_data=vals$saved_data,r_exchange=r_exchange(),replace=input$replace)
    ...
    run_exchange<-reactive({
      ...
      if(ato=="factor"){
        ...
        if(input$copy_transfer=="Move"){
          if(from!=to){
            attr(vals$saved_data[[from]],"factors")[input$importvar]<-NULL
          } else{
            if(afrom!=ato){
              attr(vals$saved_data[[from]],"factors")[input$importvar]<-NULL
            }
        ...
        if(input$copy_transfer=="Move"){
          if(from!=to){
            vals$saved_data[[from]][input$importvar]<-NULL
          }  else{
            if(afrom!=ato){
              vals$saved_data[[from]][input$importvar]<-NULL
            }
...
tool2_tab4$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ...
    observeEvent(ignoreInit = T,input$newfactorfile,{
      ...
      labels <-data.frame(data.table::fread(input$newfactorfile$datapath, stringsAsFactors = T,na.strings=c("","NA"), header=T))
...
tool2_tab10$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    output$tree_gen<-  shinyTree::renderTree({
      attrlist<-getTree_saved_data(vals,F,imesc_attrs=c("numeric","factor","coords"),imesc_models=c("pwRDA","som","kmeans",as.character(available_models)))
              ...
              div(style="height: 350px; background-color: #e3e3e3;",
                  ...
                  tabsetPanel(
                    ...
                    -webkit-box-shadow: inset 0px 0px 5px #797979ff;",
                            div("Click on the Nodes ",tipify_ui( icon(verify_fa=FALSE,name=NULL,class="fas fa-question-circle"),"Click on the nodes to expand and select the factor levels. Only available for factors with less than 100 levels"),
...
tool9$server<-function (id,vals){
  moduleServer(id,function(input, output, session){
    ...
    observe({
      ...
      print("newpalette_colors")
      print(length(vals$newpalette_colors))
    })
