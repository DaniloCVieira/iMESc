...
#' @noRd
my_layerdistances<-kohonen:::LayerDistances
my_getnNA<-kohonen:::getnNA
...
my_predict.kohonen<-kohonen:::predict.kohonen
#' @export
dtable_som_mysom<-function (kohobj, whatmap, data, classif = NULL)
{
  ...
  nNA <- my_getnNA(data, maxNA.fraction, nobjects)
  ...
  res<-lapply(1:length(units),function(i){
    ...
    d2wus<-my_layerdistances(data = datamat, codes = codemat, uclassif = kohobj_temp$unit.classif -1, numVars = nvars, numNAs = nNA, distanceFunctions = dist.ptrs,weights = weights)
...
ko_predict<-function (modelFit, newdata, submodels = NULL) {
  if(modelFit$problemType=="Regression"){
    ...
    pred <- my_predict.kohonen(modelFit, list(X = as.matrix(newdata)), whatmap = "X")$predictions$Y
    if(anyNA(pred)){
      newpred<-pred
...
#' @export
predict.kohonen2_0<-function (object, newdata = NULL, unit.predictions = NULL, trainingdata = NULL,whatmap = NULL, threshold = 0, maxNA.fraction = object$maxNA.fraction,which_best=2,dtable_train,dtable_new,...) {
  codeNcols <- sapply(object$codes, ncol)
  ncodes <- nrow(object$codes[[object$whatmap[1]]])
  if (is.null(whatmap)) {
    whatmap <- object$whatmap
...
#' @export
ko_predict_0<-function (modelFit, newdata, submodels = NULL) {
  if(modelFit$problemType=="Regression"){
    pred <- predict(modelFit, list(X = as.matrix(newdata)), whatmap = "X")$predictions$Y
    pred
