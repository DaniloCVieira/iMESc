...
#' @export
module_server_figs <- function (input, output, session,vals,file="",datalist_name=T){
  ns <- session$ns
  ...
  fn_download <- function()
  {
    ...
    switch (
      ...
      "Feature Shuffling"={plot(vals$spd_varImp_plot)},
      "Pairs-plot"={GGally:::print.ggmatrix(vals$desc_pairplot)},

      "Pairs plot"=GGally:::print.ggmatrix(vals$fm_downplot4),
    ...
    withProgress( message = "Rendering preview",
                  {
                    ...
                    list(src = paste0(vals$hand_plot,".png",sep=""),
  ...
  modal_downfig<-function(){
    showModal({
      ...
      output$header<-renderUI({
        div(p(strong("action:"),"download", vals$hand_plot),
