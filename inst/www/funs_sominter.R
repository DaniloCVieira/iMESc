...
imesc_hclutering<-function(data, k,hc_fun,hc_method,distance_metric=NULL, target="som codebook", model_name=1,whatmap=NULL,use_weights=F){
  ...
  if(class(m)[1]=="kohonen"){
   ...
   # d=object.distances(m,"codes")
    ...
    dlog<-capture_log1(vegan::vegdist)(m,distance_metric)
    d<-dlog[[1]]
  ...
  if(length(d)>0){
    hclog<-capture_log1(factoextra::hcut)(d,k,hc_func =hc_fun ,hc_method =hc_method  ,isdiss =T)
    hc<-hclog[[1]]
...
cutsom_new<-function(m, k,hc_fun,hc_method,distance_metric=NULL,use_weights=F,whatmap=NULL){
  if(class(m)[1]=="kohonen"){
    ...
    hc<-factoextra::hcut(d,k,hc_func =hc_fun ,hc_method =hc_method  ,method=hc_method,isdiss =T)
    ...
    dlog<-capture_log1(vegan::vegdist)(m,distance_metric, na.rm=T)
    d<-dlog[[1]]
    d_message<-sapply(dlog$logs,function(x) x$message)
    if(length(d_message)==0){
      d_message<-NULL
    ...
    hclog<-capture_log1(imesc_fviz_nbclust)(d,k,hc_func =hc_fun ,hc_method =hc_method  ,isdiss =T)
    hc<-hclog[[1]]
    h_message<-sapply(hclog$logs,function(x) x$message)
...
cutsom<-function(m,groups, members=NULL, method.hc="ward.D2", palette="turbo",newcolhabs=NULL, dataX,weighted=F){
  ...
  hcut<-dendextend::cutree( hc, groups)
  names(hcut)<-rownames(codes)
...
rect.dendrogram3<-function (tree, k = NULL,  horiz = FALSE,upper_rect = 0, prop_k_height = 0.5) {
  ...
  cluster <- dendextend::cutree(tree, k = k)
...
ggplot.ggdend2<-function (data = NULL, mapping = aes(), ..., segments = TRUE,labels = TRUE, nodes = TRUE, horiz = FALSE, theme = theme_dendro(),
  ...
  data <- dendextend::as.ggdend(dend1)
  data <- dendextend::prepare.ggdend(data)
...
get_dend_labposition<-function(dend,groups){
  ...
  segments<-dendextend::as.ggdend(dend)$segments
...
hc_plot<-function(obs.clusters,hc.clusters,hc.object, palette=NULL, labels=NULL, lwd=2, main="", xlab="Observations", ylab="Height", base_size=12, theme='theme_grey',offset_labels=-.1,xlab_adj=20, legend=c("outside","inside")){
  ...
  if(is.null(col)){ color=viridis::turbo(nlevels(obs.clusters))[lbls]
  } else {color=col[lbls]}
  dend1 <- dendextend::color_branches(dend=hc.dendo, col=color,k = groups, groupLabels = lbls)
  if(is.null(col)){
    colors_dend1="black"
  } else {colors_dend1="black" }

  if(!is.null(labels)){
    lab_dend<-dendextend::labels(dend1)
    names(labels)<-names(obs.clusters)
    dendextend::labels(dend1)<-labels[dendextend::labels(dend1)]
  }
  dendextend::labels_colors(dend1)<-colors_dend1
