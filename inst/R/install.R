...
# Function to detach a package if it is currently loaded
# Arguments:
#   - pkg: Name of the package to detach
#   - installed: List of installed packages (not used directly here but could be for validation)
#   - character.only: Whether the package name is passed as a character string
...
# Define required packages and their minimum versions
 ...
 # Detach packages that are not currently installed
  ...
  if(!"vegan"%in%installed){
    install.packages("vegan",verbose=F,quiet=T,type = "binary")
    message("vegan installed")
  }
  if(!"colorspace"%in%installed){
    install.packages("colorspace",verbose=F,quiet=T,type = "binary")
    message("colorspace installed")
  }
...
# Identify packages that are missing or outdated
  to_install <- pack_table[which(apply(pack_table[c('installed', 'current_version')], 1, sum) < 2),"required"]
 # Install missing or outdated packages using 'pak'
...
# Clean up temporary files created by 'pak'
