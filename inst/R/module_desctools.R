...
module_server_desctools<-function (input,output,session,vals,df_colors,newcolhabs,df_symbol ){
  ...
  bbox_y_datalist<-reactive({
    ...
    vals$saved_data[[input$bbox_y_datalist]]
  ...
  getbox<-reactive({
    ...
    data<-datay<-bbox_y_datalist()[rownames(datax),]
...
observeEvent(ignoreInit = T,
  ...
  output$stats_panels_rda<-renderUI({
    req(input$rda_X)
    validate(need(!anyNA(get_rdaX()), "This functionality does not support missing values; Please use the transformation tool to the handle missing values."))
    tabsetPanel(
  ...
  getmissing<-reactive({
    ...
    req(is.data.frame(getdata_descX()))
    data=getdata_descX()
    ...
    if(length(res0)>0){
      ...
      rownames(res)<-res[,1]
      pic<-colnames(getdata_descX())[which(colnames(getdata_descX())%in%res[,1])]
      res[,1]<-NULL
      if(length(pic)>0)
        res[pic,, drop=F]
    }
