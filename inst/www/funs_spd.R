...
plot_rf_tree <- function(final_model, tree_num, digits_split = 2, digits_pred = 1,base_size=11) {
  size=base_size/3
  tree <- randomForest::getTree(
    ...
    dplyr::mutate(
      ...
      split_label = dplyr::if_else(
        ...
        paste0(
          `split var`, "\n≤ ",
          round(`split point`, digits_split)
        )
      ),
      leaf_label = dplyr::if_else(
        is_leaf,
        paste0("Pred.\n", round(prediction, digits_pred)),
        NA_character_
      )
    )
  ...
  ggraph::ggraph(graph, layout = "tree") +
    ggraph::geom_edge_link(color = "grey40") +
    ggraph::geom_node_label(
      ggplot2::aes(label = split_label),
      na.rm = TRUE,
      fill = "grey95",
      color = "black",
      size = size
